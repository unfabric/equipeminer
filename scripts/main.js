...
'use strict';{function IsVideoPlaying(elem){return elem&&!elem.paused&&!elem.ended&&elem.currentTime>0}const DOM_COMPONENT_ID="video";const HANDLER_CLASS=class VideoDOMHandler extends self.DOMElementHandler{constructor(iRuntime){super(iRuntime,DOM_COMPONENT_ID);this.SetAutoAttach(false);this._isRuntimeInWorker=false;this._supportsRvfc=false;this._postImageBitmaps=false;this._sequenceNumber=0;this._numVideos=0;this._destroyedVideos=new WeakSet;this.AddRuntimeMessageHandler("init",e=>this._OnInit(e));
this.AddDOMElementMessageHandler("play",elem=>this._OnPlay(elem));this.AddDOMElementMessageHandler("pause",elem=>this._OnPause(elem));this.AddDOMElementMessageHandler("set-source",(elem,e)=>this._OnSetSource(elem,e));this.AddDOMElementMessageHandler("set-playback-time",(elem,e)=>this._OnSetPlaybackTime(elem,e));this.AddDOMElementMessageHandler("set-playback-rate",(elem,e)=>this._OnSetPlaybackRate(elem,e));this.AddDOMElementMessageHandler("set-looping",(elem,e)=>this._OnSetLooping(elem,e));this.AddDOMElementMessageHandler("set-muted",
(elem,e)=>this._OnSetMuted(elem,e));this.AddDOMElementMessageHandler("set-volume",(elem,e)=>this._OnSetVolume(elem,e));self["C3Video_GetElement"]=elementId=>this.GetElementById(elementId)}async _OnInit(e){this._isRuntimeInWorker=e["isInWorker"];const tmpVideo=document.createElement("video");this._supportsRvfc=!!tmpVideo["requestVideoFrameCallback"]&&!this._iRuntime.IsAndroidWebView();const supportsImageBitmap=!!self["createImageBitmap"];this._postImageBitmaps=this._isRuntimeInWorker||supportsImageBitmap;
return{"postImageBitmaps":this._postImageBitmaps,"supportedFormats":{"webm-vp8":!!tmpVideo.canPlayType("video/webm;codecs=vp8"),"webm-vp9":!!tmpVideo.canPlayType("video/webm;codecs=vp9"),"webm-av1":!!tmpVideo.canPlayType("video/webm;codecs=av01.0.00M.08"),"mp4-h264":!!tmpVideo.canPlayType("video/mp4;codecs=avc1.420034"),"mp4-h265":!!tmpVideo.canPlayType("video/mp4;codecs=hev1.1.2.L93.B0"),"mp4-av1":!!tmpVideo.canPlayType("video/mp4;codecs=av01.0.00M.08")}}}TryPlayMedia(mediaElem){this._iRuntime.TryPlayMedia(mediaElem);
if(this._postImageBitmaps&&this._supportsRvfc&&mediaElem["c3_rvfcId"]===-1)mediaElem["c3_rvfcId"]=mediaElem["requestVideoFrameCallback"](()=>this._OnVideoFrameCallback(mediaElem))}RemovePendingPlay(mediaElem){this._iRuntime.RemovePendingPlay(mediaElem);if(this._postImageBitmaps&&this._supportsRvfc&&mediaElem["c3_rvfcId"]!==-1){mediaElem["cancelVideoFrameCallback"](mediaElem["c3_rvfcId"]);mediaElem["c3_rvfcId"]=-1}}async _OnVideoFrameCallback(mediaElem){if(this._destroyedVideos.has(mediaElem))return;
mediaElem["c3_rvfcId"]=mediaElem["requestVideoFrameCallback"](()=>this._OnVideoFrameCallback(mediaElem));try{const sequenceNumber=mediaElem["c3_sequenceNumber"]++;const imageBitmap=await createImageBitmap(mediaElem);if(sequenceNumber>mediaElem["c3_lastFrameSequence"]){mediaElem["c3_imageBitmap"]=imageBitmap;mediaElem["c3_lastFrameSequence"]=sequenceNumber}}catch(err){}}CreateElement(elementId,e){const elem=document.createElement("video");elem.crossOrigin="anonymous";elem["playsInline"]=true;elem["c3_imageBitmap"]=
null;elem["c3_rvfcId"]=-1;elem["c3_sequenceNumber"]=0;elem["c3_lastFrameSequence"]=-1;elem.addEventListener("canplay",()=>this._SendPlaybackEvent(elem,elementId,0));elem.addEventListener("canplaythrough",()=>this._SendPlaybackEvent(elem,elementId,1));elem.addEventListener("ended",()=>this._SendPlaybackEvent(elem,elementId,2));elem.addEventListener("error",()=>this._SendPlaybackEvent(elem,elementId,3));elem.addEventListener("loadstart",()=>this._SendPlaybackEvent(elem,elementId,4));elem.addEventListener("playing",
()=>this._SendPlaybackEvent(elem,elementId,5));elem.addEventListener("pause",()=>this._SendPlaybackEvent(elem,elementId,6));elem.addEventListener("stalled",()=>this._SendPlaybackEvent(elem,elementId,7));const autoplay=e["autoplay"];if(autoplay===0){elem.autoplay=false;elem.preload="none"}else if(autoplay===1){elem.autoplay=false;elem.preload="auto"}else elem.autoplay=true;if(e["src"])elem.src=e["src"];if(autoplay===2)this.TryPlayMedia(elem);this._numVideos++;this._StartTicking();return elem}_SendPlaybackEvent(elem,
elementId,num){if(this._destroyedVideos.has(elem))return;this.PostToRuntimeElement("playback-event",elementId,{"type":num})}UpdateState(elem,e){}DestroyElement(elem){this.RemovePendingPlay(elem);if(IsVideoPlaying(elem))elem.pause();elem.src="";this._numVideos=Math.max(this._numVideos-1,0);if(this._numVideos===0)this._StopTicking();this._destroyedVideos.add(elem)}_OnPlay(elem){this.TryPlayMedia(elem)}_OnPause(elem){this.RemovePendingPlay(elem);elem.pause()}_OnSetSource(elem,e){elem.src=e["src"];elem.load()}_OnSetPlaybackTime(elem,
e){try{elem.currentTime=e["time"]}catch(err){console.error("[Video] Exception setting playback time: ",err)}}_OnSetPlaybackRate(elem,e){try{elem.playbackRate=e["rate"]}catch(err){console.error("[Video] Exception setting playback rate: ",err)}}_OnSetLooping(elem,e){elem.loop=e["isLooping"]}_OnSetMuted(elem,e){elem.muted=e["isMuted"]}_OnSetVolume(elem,e){elem.volume=e["volume"]}Tick(){const videoData={};const stateData={"sequenceNumber":this._sequenceNumber++,"videoData":videoData};const transferables=
[];const promises=[];for(const [id,elemState]of this._elementMap.entries()){const elem=elemState.GetElement();const o={"currentTime":elem.currentTime,"playbackRate":elem.playbackRate,"duration":elem.duration,"videoWidth":elem.videoWidth,"videoHeight":elem.videoHeight};if(this._postImageBitmaps)if(this._supportsRvfc){const imageBitmap=elem["c3_imageBitmap"];elem["c3_imageBitmap"]=null;o["imageBitmap"]=imageBitmap;if(imageBitmap)transferables.push(imageBitmap)}else promises.push(createImageBitmap(elem).then(imageBitmap=>
{o["imageBitmap"]=imageBitmap;transferables.push(imageBitmap)}).catch(err=>{o["imageBitmap"]=null}));videoData[id.toString()]=o}Promise.all(promises).then(()=>{if(this._isRuntimeInWorker)this.PostToRuntime("state",stateData,false,transferables);else this._PostToRuntimeMaybeSync("state",stateData)})}};self.RuntimeInterface.AddDOMHandlerClass(HANDLER_CLASS)};
